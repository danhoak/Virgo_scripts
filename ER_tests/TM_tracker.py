...
def data_demod(t,xx,f0,Fs,duration,lp=6000.0,Fs0=10000.0):
    ...
    #Ipass = lowpass(Iphase,1.0,Fs0)
    #Qpass = lowpass(Qphase,1.0,Fs0)
    #xdemod = sqrt(Ipass**2 + Qpass**2)
    xdemod = sqrt(Iphase**2 + Qphase**2)
...
err_chan = 'V1:SSFS_B4_Err_pre_50kHz'
...
for i in range(len(t_DARM)):
    ...
    pre_line, t_dec = data_demod(ts,x.data,f0,Fs,step_duration,lp=10000.0,Fs0=25000.0)

    datapoints[i] = median(pre_line)*100
...
pylab.legend(('Demodulated data, 7809.62 Hz','Exponential Fit, Q = %.0f +/- %.0f'%(Q, Qerr)),fancybox=True,prop={'size':10})
pylab.title('TM Drum Mode Ringdown, GPS = %.0f'%start_time, fontsize=12)
