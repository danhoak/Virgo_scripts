...
for i in range(len(t_DARM)):
    ...
    pre_line, t_dec = data_demod(ts,x.data,f0,Fs,step_duration,lp=500.0,Fs0=2000.0)
...
pylab.grid(True)
pylab.legend(('Demodulated data, 311.23 Hz','Exponential Fit, Q = %.0f +/- %.0f'%(Q, Qerr)),fancybox=True,prop={'size':10})
pylab.title('Test Mass Violin Mode Ringdown, GPS = %.0f'%start_time, fontsize=12)
...
pylab.savefig('DARM_violin_fit.png')
